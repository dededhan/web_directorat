...
class ManageUserController extends Controller
{
    ...
    {
        // Only show fakultas, prodi, and admin_pemeringkatan roles
        $users = User::whereIn('role', ['fakultas', 'prodi', 'admin_pemeringkatan'])
            ->latest()
            ->get();
        
        $facultiesAndProgramsData = $this->getFacultiesAndProgramsData();
        
        Log::info('Admin Pemeringkatan fetched users for manage page', ['count' => $users->count()]);
    ...
    public function store(Request $request) 
    {
        ...
        $validated = $request->validate([
            ...
            'role' => ['required', 'string', Rule::in(['fakultas', 'prodi', 'admin_pemeringkatan'])],
        ]);
        ...
        try {
            ...
            DB::commit();
            Log::info('User created by admin_pemeringkatan', [
                'user_id' => $user->id, 
                'role' => $user->role, 
                'name' => $user->name,
                'created_by' => auth()->id()
    ...
    {
        // Only allow toggling status for fakultas, prodi, admin_pemeringkatan
        if (!in_array($user->role, ['fakultas', 'prodi', 'admin_pemeringkatan'])) {
            return redirect()->back()->with('error', 'Anda tidak memiliki akses untuk mengubah status user ini.');
        }
    ...
    {
        // Only allow updating fakultas, prodi, admin_pemeringkatan accounts
        if (!in_array($user->role, ['fakultas', 'prodi', 'admin_pemeringkatan'])) {
        ...
        try {
            ...
            Log::info('User updated by admin_pemeringkatan', [
                'user_id' => $user->id, 
                'new_role' => $user->role, 
                'new_name' => $user->name,
                'updated_by' => auth()->id()
            ...
            return redirect()->route('admin_pemeringkatan.manageuser.index')
                ->with('success', 'User berhasil diperbarui: ' . $user->name);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    {
        Log::debug('Admin Pemeringkatan: Received delete request for user ID: ' . $id);
        $user = User::find($id);
        ...
        // Only allow deleting fakultas, prodi, admin_pemeringkatan accounts
        if (!in_array($user->role, ['fakultas', 'prodi', 'admin_pemeringkatan'])) {
        ...
        try {
            ...
            Log::info('User deleted by admin_pemeringkatan', [
                'user_id' => $id, 
                'name' => $userName,
                'deleted_by' => auth()->id()
            ]);
