...
document.getElementById('searchInput')?.addEventListener('keyup', function() {
    ...
    const rows = document.querySelector('#akreditasi-list').getElementsByTagName('tr');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.delete-btn').forEach(button => {
        button.addEventListener('click', function() {
            ...
            showConfirmationDialog('Apakah Anda yakin ingin menghapus data akreditasi ini?', () => {
                form.submit();
            });
        });
    });
    ...
    document.querySelectorAll('.edit-akreditasi').forEach(button => {
        button.addEventListener('click', function() {
            const akreditasiId = this.dataset.id;

            // Fetch akreditasi details via AJAX
            fetch(`/admin/dataakreditasi/${akreditasiId}/detail`)
                ...
                .then(data => {
                    ...
                    document.getElementById('edit_lembaga_akreditasi').value = data.lembaga_akreditasi;
                    document.getElementById('edit_peringkat').value = data.peringkat;
                    document.getElementById('edit_nomor_sk').value = data.nomor_sk;
                    ...
                    // Set the form action
                    const form = document.getElementById('editAkreditasiForm');
                    form.action = `/admin/dataakreditasi/${akreditasiId}`;

                    // Show the modal
                    new bootstrap.Modal(document.getElementById('editAkreditasiModal')).show();
                })
                .catch(error => {
                    console.error('Error fetching akreditasi details:', error);
                    showErrorAlert('Gagal mengambil data akreditasi.');
                });
        });
    });

    // Handle save button click
    document.getElementById('saveEditAkreditasi')?.addEventListener('click', function() {
        const form = document.getElementById('editAkreditasiForm');
        ...
        .then(data => {
            if (data.success) {
                // Close the modal
                bootstrap.Modal.getInstance(document.getElementById('editAkreditasiModal'))
                    .hide();

                // Show success message
                showSuccessAlert(data.message || 'Data akreditasi berhasil diperbarui!');
        ...
        .catch(error => {
            console.error('Error saving akreditasi data:', error);
