...
if (!function_exists('getStatusInfoAdmin')) {
    function getStatusInfoAdmin($status) {
        switch ($status) {
            case 'diproses': return ['color' => 'blue', 'icon' => 'bx-loader-alt', 'text' => 'Diproses'];
...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100" 
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            ...
            <div class="p-6 lg:p-8 space-y-8">
                ...
                {{-- Ditolak Notes --}}
    @if ($submission->status == 'ditolak' && !empty($submission->catatan_revisi))
    <div class="bg-red-50 border-l-4 border-red-400 p-4 rounded-lg shadow-sm" role="alert">
        <div class="flex items-start">
            ...
            <div class="ml-3">
                <h3 class="text-sm font-bold text-red-800">Catatan Penolakan dari Admin</h3>
                <p class="text-sm text-red-700 mt-1 whitespace-pre-wrap">{{ $submission->catatan_revisi }}</p>
            </div>
        </div>
    </div>
    @endif
                    ...
                    <form action="{{ route('admin_equity.apc.submission.updateStatus', $submission->id) }}" method="POST" class="space-y-4">
                        ...
                        <div>
                            ...
                            <select name="status" id="status" 
                                ...
                                <option value="diproses" @if($submission->status == 'diproses') selected @endif>Diproses</option>
                        ...
                        {{-- Conditional Catatan Penolakan Textarea --}}
        <div x-show="selectedStatus === 'ditolak'" 
             x-transition
             style="display: none;">
            <label for="catatan_revisi" class="block text-xs font-bold uppercase text-gray-500 mb-2">Catatan Penolakan (Wajib diisi)</label>
            <textarea name="catatan_revisi" id="catatan_revisi" rows="4" 
                      class="w-full rounded-xl border-gray-300 shadow-sm focus:border-teal-500 focus:ring-teal-500 text-sm" 
                      placeholder="Jelaskan alasan penolakan pengajuan ini...">{{ old('catatan_revisi', $submission->catatan_revisi) }}</textarea>
