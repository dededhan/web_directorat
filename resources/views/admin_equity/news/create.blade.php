...
@section('title', 'Tambah Berita EQUITY')
...
<div class="px-6 py-4">
    ...
    <div class="bg-white rounded-lg shadow-md overflow-hidden">
        <div class="bg-gradient-to-r from-teal-600 to-teal-700 px-6 py-4">
            <h2 class="text-xl font-bold text-white">Tambah Berita & Informasi Terkini</h2>
            <p class="text-teal-100 text-sm mt-1">Isi form di bawah untuk menambahkan berita baru</p>
        </div>
        <div class="p-6">
            @if ($errors->any())
            ...
            <form action="{{ route('admin_equity.news.store') }}" method="POST" enctype="multipart/form-data" onsubmit="console.log('Form submitting...')">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent @error('gradient_color') border-red-500 @enderror" 
                                id="gradient_color" 
                                name="gradient_color" 
                                required>
                            <option value="">Pilih Warna</option>
                            @foreach($gradientColors as $key => $label)
                                <option value="{{ $key }}" {{ old('gradient_color') == $key ? 'selected' : '' }}>
                                    {{ $label }}
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="mb-6">
                    ...
                    <textarea class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent @error('description') border-red-500 @enderror" 
                              id="description" 
                              name="description" 
                              rows="8"
                              placeholder="Deskripsi singkat berita (opsional)">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="mb-6">
                    ...
                    <input type="file" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent @error('image') border-red-500 @enderror" 
                           id="image" 
                           name="image" 
                    ...
                    @error('image')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                    <p class="mt-1 text-xs text-gray-500">Format: JPG, PNG, GIF. Maksimal 2MB. Resolusi rekomendasi: 800×600px</p>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                        ...
                        <input type="number" 
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-transparent @error('order') border-red-500 @enderror" 
                               id="order" 
                               name="order" 
                               value="{{ old('order', 0) }}" 
                               min="0">
                ...
                <div class="flex gap-3 pt-4 border-t border-gray-200">
                    <button type="submit" class="inline-flex items-center px-6 py-2 bg-teal-600 hover:bg-teal-700 text-white font-semibold rounded-lg shadow-md transition duration-150">
