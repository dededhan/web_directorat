        ...
        <header class="mb-8">
            <nav class="text-sm text-gray-500 mb-3" aria-label="Breadcrumb">
                <ol class="list-none p-0 inline-flex items-center space-x-2">
                    ...
                    <li><i class='bx bx-chevron-right text-base text-gray-400'></i></li>
                    <li class="font-medium text-gray-800">Buat Pengajuan Baru</li>
                </ol>
            </nav>
            <div>
                <h1 class="text-2xl lg:text-3xl font-bold text-gray-800">Formulir Pengajuan Insentif</h1>
                <p class="mt-2 text-gray-600">Isi detail pengajuan insentif untuk reviewer.</p>
            </div>
        </header>

        {{-- Form --}}
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100">
            <form method="POST" action="{{ route('admin_equity.incentivereviewer.store') }}">
                @csrf
                <div class="p-6 sm:p-8 space-y-6">
                    <div>
                        <label for="nama_reviewer" class="block text-sm font-medium text-gray-700 mb-2">Nama Reviewer</label>
                        <input type="text" name="nama_reviewer" id="nama_reviewer" class="w-full text-gray-800 bg-gray-50 border border-gray-200 rounded-lg p-3" placeholder="Masukkan nama lengkap reviewer" required>
                    </div>

                    <div>
                        <label for="judul_artikel" class="block text-sm font-medium text-gray-700 mb-2">Judul Artikel yang Direview</label>
                        <input type="text" name="judul_artikel" id="judul_artikel" class="w-full text-gray-800 bg-gray-50 border border-gray-200 rounded-lg p-3" placeholder="Masukkan judul artikel" required>
                    </div>

                    <div>
                        <label for="tanggal_pengajuan" class="block text-sm font-medium text-gray-700 mb-2">Tanggal Pengajuan</label>
                        <input type="date" name="tanggal_pengajuan" id="tanggal_pengajuan" class="w-full text-gray-800 bg-gray-50 border border-gray-200 rounded-lg p-3" required>
                    </div>
