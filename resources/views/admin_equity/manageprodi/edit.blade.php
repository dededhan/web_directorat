...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        <header class="mb-8">
            <nav class="mb-4" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-2 text-sm">
                    ...
                    <li class="flex items-center">
                        <i class='bx bx-chevron-right text-gray-400 mx-2'></i>
                        <span class="font-medium text-gray-800">Edit Prodi</span>
                    </li>
                </ol>
            </nav>
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:p-8">
                <div class="flex items-start space-x-4">
                    ...
                    <div>
                        <h1 class="text-2xl lg:text-3xl font-bold text-gray-900 mb-2">Edit Prodi</h1>
                        <p class="text-gray-600 text-base lg:text-lg">Perbarui data prodi: {{ $prodi->name }}</p>
                    </div>
                </div>
            </div>
        </header>
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <form method="POST" action="{{ route('admin_equity.manageprodi.update', $prodi->id) }}">
                @csrf
                @method('PUT')
                    ...
                    <div>
                        ...
                        <select id="fakultas_id" name="fakultas_id"
                            ...
                            @foreach($fakultas as $fak)
                                <option value="{{ $fak->id }}" @if(old('fakultas_id', $prodi->fakultas_id) == $fak->id) selected @endif>
                                    {{ $fak->name }}
                                </option>
                            @endforeach
                        </select>
