...
@section('contentadmin_pemeringkatan')
            ...
            <div class="bg-white rounded-xl shadow-lg overflow-hidden">
                ...
                <div class="bg-gradient-to-r from-amber-600 to-amber-700 px-6 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-2xl font-bold text-white mb-1">Edit Mata Kuliah Sustainability</h1>
                            <p class="text-amber-100 text-sm">Perbarui informasi mata kuliah sustainability</p>
                        </div>
                ...
                <!-- Form -->
                <form action="{{ route('admin_pemeringkatan.mata-kuliah-sustainability.update', $matakuliah->id) }}" method="POST" enctype="multipart/form-data" class="p-6 sm:p-8">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="lg:col-span-2">
                                ...
                                <select name="sdgs_group" 
                                    ...
                                    @foreach ($sdgGoalsData as $number => $description)
                                        ...
                                        <option value="{{ $optionValue }}" {{ old('sdgs_group', $matakuliah->sdgs_group) == $optionValue ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="fakultas" 
                                    ...
                                    @foreach ($faculties as $key => $faculty)
                                        <option value="{{ strtolower($key) }}" {{ old('fakultas', $matakuliah->fakultas) == strtolower($key) ? 'selected' : '' }}>
                                            {{ $faculty['name'] }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="mb-6">
                            ...
                            @if($matakuliah->rps_path)
                                <div class="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                                    <p class="text-sm text-gray-700 mb-2">File RPS saat ini:</p>
                                    <a href="{{ Storage::url($matakuliah->rps_path) }}" 
                                       target="_blank"
                                       class="inline-flex items-center text-blue-600 hover:text-blue-800 font-medium">
                                        <i class='bx bx-file-blank text-lg mr-1'></i>
                                        {{ basename($matakuliah->rps_path) }}
                                    </a>
                                </div>
                            @endif
                            <input type="file" 
                            ...
                            <p class="text-xs text-gray-500 mt-1">Biarkan kosong jika tidak ingin mengubah file RPS. Upload file baru (PDF, DOC, DOCX, maksimal 10MB)</p>
                            @error('rps')
                                <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const currentProdi = "{{ old('prodi', $matakuliah->prodi) }}";
