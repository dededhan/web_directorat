...
<div class="px-4 sm:px-6 lg:px-8 py-8" x-data="{ activeTab: 'questions', isImportModalOpen: false }">
    <div>
                ...
                <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                    ...
                    <a href="#" @click.prevent="activeTab = 'add_question'" 
                     ...
                     <a href="#" @click.prevent="isImportModalOpen = true" 
                       ...
                       <i class="fas fa-file-upload mr-2"></i>Import Soal
                    </a>
                </nav>
                @if(count($questionBank->questions) > 0)
                <button @click="confirmClearAll()" class="mb-2 inline-flex items-center px-3 py-2 border border-red-300 shadow-sm text-sm leading-4 font-medium rounded-md text-red-700 bg-white hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
    ...
    <div x-show="isImportModalOpen" x-transition
        ...
        <div @click.away="isImportModalOpen = false" class="bg-white rounded-lg shadow-xl w-full max-w-lg">
            ...
            <form action="{{ route('admin_pemeringkatan.question_banks.import', $questionBank->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
    ...
    function confirmClearAll() {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                form.method = 'POST';
                form.action = '{{ route("admin_pemeringkatan.question_banks.clear_all", $questionBank->id) }}';
