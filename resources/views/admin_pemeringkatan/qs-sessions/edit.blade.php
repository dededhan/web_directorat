...
<div class="p-6 max-w-4xl mx-auto space-y-6">
    ...
    <div>
        <nav class="flex text-sm text-gray-500 mb-2" aria-label="Breadcrumb">
            ...
            <a href="{{ route('admin_pemeringkatan.qs-sessions.show', $qs_session) }}" class="hover:text-teal-600 transition">
                {{ $qs_session->name }}
            </a>
            <span class="mx-2">/</span>
            <span class="text-gray-800 font-medium">Edit Sesi</span>
        </nav>
        <h1 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
            <i class="fas fa-edit text-teal-600"></i>
            Edit Sesi Campaign: {{ $qs_session->name }}
        </h1>
        <p class="text-sm text-gray-500 mt-1">
            Perbarui informasi nama, tanggal pelaksanaan, atau ubah status sesi kampanye.
        </p>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 md:p-8">
        ...
        <form action="{{ route('admin_pemeringkatan.qs-sessions.update', $qs_session) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <input type="text" name="name" id="name" required
                       value="{{ old('name', $qs_session->name) }}"
            ...
            <div>
                ...
                <textarea name="description" id="description" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">{{ old('description', $qs_session->description) }}</textarea>
            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <label class="relative flex flex-col p-4 bg-white border-2 rounded-xl cursor-pointer hover:border-teal-500 transition-all has-[:checked]:border-teal-600 has-[:checked]:bg-teal-50/20 shadow-sm">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <input type="radio" name="mode" value="consent_only" class="w-4 h-4 text-teal-600" {{ old('mode', $qs_session->mode ?? 'consent_only') === 'consent_only' ? 'checked' : '' }}>
                        </div>
                    ...
                    <label class="relative flex flex-col p-4 bg-white border-2 rounded-xl cursor-pointer hover:border-teal-500 transition-all has-[:checked]:border-teal-600 has-[:checked]:bg-teal-50/20 shadow-sm">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <input type="radio" name="mode" value="form_based" class="w-4 h-4 text-teal-600" {{ old('mode', $qs_session->mode ?? 'consent_only') === 'form_based' ? 'checked' : '' }}>
                        </div>
                        <p class="text-xs text-gray-500 leading-relaxed">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="status" id="status" required
                        ...
                        <option value="draft" {{ old('status', $qs_session->status) === 'draft' ? 'selected' : '' }}>Draft (Persiapan)</option>
                        <option value="active" {{ old('status', $qs_session->status) === 'active' ? 'selected' : '' }}>Active (Aktif)</option>
                        <option value="closed" {{ old('status', $qs_session->status) === 'closed' ? 'selected' : '' }}>Closed (Selesai/Tutup)</option>
                    </select>
                    <p class="text-xs text-amber-600 mt-1">
                        <i class="fas fa-info-circle mr-1"></i> Jika ditutup (Closed), semua responden pending akan otomatis expired.
                    </p>
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" id="start_date"
                           value="{{ old('start_date', $qs_session->start_date ? $qs_session->start_date->format('Y-m-d') : '') }}"
                ...
                <div>
                    ...
                    <input type="date" name="end_date" id="end_date"
                           value="{{ old('end_date', $qs_session->end_date ? $qs_session->end_date->format('Y-m-d') : '') }}"
                           class="w-full px-3 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-teal-500 focus:border-transparent">
                </div>
            </div>
            ...
            <div class="pt-6 border-t border-gray-100 flex items-center justify-end space-x-3">
                <a href="{{ route('admin_pemeringkatan.qs-sessions.show', $qs_session) }}" 
