...
@section('contentadmin_inovasi')
    <div class="max-w-2xl mx-auto space-y-6">
        ...
        <div class="bg-white rounded-xl shadow border border-gray-100 p-6">
            <form action="{{ route('admin_inovasi.accounts.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div>
                    ...
                    <select id="role" name="role"
                        ...
                        <option value="admin_inovasi" {{ old('role') === 'admin_inovasi' ? 'selected' : '' }}>Admin Inovasi
                        </option>
                        ...
                        <option value="pppk" {{ old('role') === 'pppk' ? 'selected' : '' }}>PPPK</option>
                        <option value="mahasiswa" {{ old('role') === 'mahasiswa' ? 'selected' : '' }}>Mahasiswa</option>
                        <option value="reviewer_inovchalenge"
                            {{ old('role') === 'reviewer_inovchalenge' ? 'selected' : '' }}>Reviewer Inov Challenge
                        </option>
                        <option value="validator" {{ old('role') === 'validator' ? 'selected' : '' }}>Reviewer Katsinov
                        </option>
                        <option value="registered_user" {{ old('role') === 'registered_user' ? 'selected' : '' }}>Google
                            User</option>
                    </select>
                    @error('role')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
