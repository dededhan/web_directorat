    ...
    <div class="space-y-6">
        ...
        <div class="flex gap-2 border-b border-gray-200 pb-0">
            ...
            'declined' => ['label' => 'Ditolak', 'icon' => 'fa-times-circle', 'color' => 'red'],
                ...
                @endphp
                <a href="{{ route('admin_inovasi.accounts.registrations', ['status' => $statusKey ?: 'pending']) }}"
        ...
        {{-- Search --}}
        <form action="{{ route('admin_inovasi.accounts.registrations') }}" method="GET" class="flex gap-2">
                ...
                <form id="batchApproveForm" action="{{ route('admin_inovasi.accounts.registrations.batchApprove') }}"
                    ...
                    <div x-show="selectedIds.length > 0" x-transition
                        ...
                        <button type="submit"
                            ...
                            onclick="return confirm('Yakin ingin menyetujui ' + document.querySelectorAll('.batch-checkbox:checked').length + ' pendaftaran yang dipilih?')">
                    ...
                    <div class="bg-white rounded-xl shadow border border-gray-100 p-5" x-data="{ showNotes: false }">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                            ...
                            <div class="flex items-start gap-4">
                                @if ($reg->status === 'pending')
                                    <div class="flex items-center pt-1">
                                        <input type="checkbox" form="batchApproveForm" name="registration_ids[]" value="{{ $reg->id }}"
                            ...
                            <div class="flex items-center gap-2 flex-shrink-0">
                                @if ($reg->status === 'pending')
                                    {{-- Approve --}}
                                    <form action="{{ route('admin_inovasi.accounts.registrations.approve', $reg) }}"
                                        method="POST"
                        ...
                        <div x-show="showNotes" x-transition class="mt-4 pt-4 border-t border-gray-200" x-cloak>
                            <form action="{{ route('admin_inovasi.accounts.registrations.decline', $reg) }}" method="POST">
                                @csrf
                                @method('PATCH')
