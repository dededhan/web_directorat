    ...
    <div class="space-y-6" x-data="{ viewMode: 'card' }">
        ...
        <div class="bg-gradient-to-r from-gray-800 to-gray-900 rounded-2xl p-6 shadow-xl text-white">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <a href="{{ route('admin_inovasi.accounts.create') }}"
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                    <a href="{{ route('admin_inovasi.accounts.index', request()->only('search')) }}"
                    ...
                    @foreach ($roleLabels as $roleKey => $label)
                        <a href="{{ route('admin_inovasi.accounts.index', array_merge(request()->only('search'), ['role' => $roleKey])) }}"
                            class="px-3.5 py-1.5 rounded-lg text-xs font-semibold transition border
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                    <form action="{{ route('admin_inovasi.accounts.index') }}" method="GET" class="flex gap-2">
                        @if (request('role'))
                ...
                <div
                    ...
                    <div class="p-5">
                        ...
                        <div class="flex items-center gap-2 mt-3 pt-3 border-t border-gray-100">
                            <a href="{{ route('admin_inovasi.accounts.edit', $user) }}"
                            ...
                            @if ($user->id !== auth()->id())
                                <form action="{{ route('admin_inovasi.accounts.destroy', $user) }}" method="POST"
                                    class="flex-1"
                            ...
                            <tr class="hover:bg-gray-50/50 transition group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center justify-center gap-1.5">
                                        ...
                                        @if ($user->id !== auth()->id())
                                            <form action="{{ route('admin_inovasi.accounts.destroy', $user) }}"
                                                method="POST"
                                                onsubmit="return confirm('Yakin ingin menghapus akun {{ $user->name }}?')">
                                                @csrf
                                                @method('DELETE')
