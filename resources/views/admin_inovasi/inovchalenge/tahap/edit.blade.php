        ...
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav class="mb-6">
                <ol class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <li><i class="fas fa-chevron-right text-xs"></i></li>
                    <li><a href="{{ route('admin_inovasi.inovchalenge.sessions.show', $tahap->session) }}"
                            class="hover:text-teal-600">{{ $tahap->session->nama_sesi }}</a></li>
                    <li><i class="fas fa-chevron-right text-xs"></i></li>
            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden mb-6">
                <div
                    class="bg-gradient-to-r {{ $tahap->tahap_ke === 1 ? 'from-blue-500 to-blue-600' : ($tahap->tahap_ke === 2 ? 'from-purple-500 to-purple-600' : 'from-orange-500 to-orange-600') }} px-6 py-4">
                    <h2 class="text-white font-semibold text-lg">
                        <i class="fas fa-flag mr-2"></i> {{ $tahap->nama_tahap }} — Pengaturan
                    </h2>
                </div>
                <form action="{{ route('admin_inovasi.inovchalenge.tahap.update', $tahap) }}" method="POST"
                    class="p-6 space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            <label for="periode_awal" class="block text-sm font-medium text-gray-700 mb-1">Periode
                                Awal</label>
                        ...
                        <div>
                            <label for="periode_akhir" class="block text-sm font-medium text-gray-700 mb-1">Periode
                                Akhir</label>
                    ...
                    @if ($tahap->has_anggota || $tahap->has_fakultas)
                        <div class="flex gap-3 text-xs text-gray-500">
                            @if ($tahap->has_anggota)
                                <span
                                    class="inline-flex items-center px-2 py-1 bg-blue-50 text-blue-700 rounded border border-blue-200">
                                    <i class="fas fa-users mr-1"></i> Anggota Tim (aktif)
                                </span>
                            @endif
                            @if ($tahap->has_fakultas)
                                <span
                                    class="inline-flex items-center px-2 py-1 bg-green-50 text-green-700 rounded border border-green-200">
            ...
            <div class="space-y-6">
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <div class="bg-gradient-to-r from-gray-700 to-gray-800 px-6 py-4 flex items-center justify-between">
                        <h2 class="text-white font-semibold text-lg">
                            ...
                            <span
                                class="ml-2 bg-white/20 text-white text-sm px-3 py-0.5 rounded-full">{{ $tahap->sections->count() }}
                                seksi</span>
                        </h2>
                    ...
                    <div x-show="showAddSection" x-transition class="p-5 border-t border-gray-100 bg-gray-50">
                        <form action="{{ route('admin_inovasi.inovchalenge.tahap.sections.store', $tahap) }}"
                            method="POST">
                            @csrf
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-3">
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-1">Judul Seksi <span
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden"
                        ...
                        <div class="bg-gradient-to-r from-indigo-500 to-indigo-600 px-6 py-3">
                            <div x-show="!editingSection" class="flex items-center justify-between">
                                <div>
                                    <h3 class="text-white font-semibold text-[15px]">
                                        ...
                                        <span
                                            class="ml-2 text-indigo-200 text-xs font-normal">({{ $section->fields->count() }}
                                            field)</span>
                                    </h3>
                                ...
                                <div class="flex gap-2">
                                    ...
                                    <form method="POST"
                                        action="{{ route('admin_inovasi.inovchalenge.tahap.sections.destroy', $section) }}"
                            ...
                            <div x-show="editingSection" x-transition>
                                <form method="POST"
                                    action="{{ route('admin_inovasi.inovchalenge.tahap.sections.update', $section) }}">
                                    @csrf @method('PUT')
                                ...
                                <div class="p-4 hover:bg-gray-50 transition group" x-data="{ editing: false, editOptions: {{ json_encode($field->field_options ?? []) }}, editType: '{{ $field->field_type }}' }">
                                    ...
                                    <div x-show="!editing" class="flex items-center justify-between">
                                        <div class="flex items-center space-x-4">
                                            <span
                                                class="flex items-center justify-center w-8 h-8 bg-gray-100 rounded-lg text-sm font-medium text-gray-500">{{ $field->urutan }}</span>
                                            <div>
                                                ...
                                                <div class="flex items-center gap-2 mt-0.5">
                                                    ...
                                                    {{ $field->field_type === 'file'
                                                        ...
                                                        @if ($field->field_type === 'checkbox')
                                                            <i class="fas fa-check-square mr-1 text-xs"></i> Multiple
                                                        @elseif($field->field_type === 'dropdown')
                                                            <i class="fas fa-caret-down mr-1 text-xs"></i> Dropdown
                                                            @else{{ ucfirst($field->field_type) }}
                                                        @endif
                                                    </span>
                                                    @if (in_array($field->field_type, ['dropdown', 'checkbox']) && $field->field_options)
                                                        <span
                                                            class="text-xs text-gray-400">({{ count($field->field_options) }}
                                                            opsi)</span>
                                                    @endif
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex items-center space-x-1">
                                            ...
                                            <form method="POST"
                                                action="{{ route('admin_inovasi.inovchalenge.tahap.fields.destroy', $field) }}"
                                    ...
                                    <div x-show="editing" x-transition>
                                        <form
                                            action="{{ route('admin_inovasi.inovchalenge.tahap.fields.update', $field) }}"
                                            method="POST" class="space-y-3">
                                            @csrf @method('PUT')
                                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                                <div>
                                                    <label
                                                        class="block text-xs font-medium text-gray-500 mb-1">Label</label>
                                                ...
                                                <div>
                                                    <label
                                                        class="block text-xs font-medium text-gray-500 mb-1">Tipe</label>
                                                ...
                                                <div>
                                                    <label
                                                        class="block text-xs font-medium text-gray-500 mb-1">Wajib?</label>
                                                    <select name="is_required"
                                                        class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-teal-500 focus:border-teal-500 text-sm">
                                                        <option value="1"
                                                            {{ $field->is_required ? 'selected' : '' }}>Ya</option>
                                                        <option value="0"
                                                            {{ !$field->is_required ? 'selected' : '' }}>Tidak</option>
                                                    </select>
                                                </div>
                                            </div>
                                            <div x-show="editType === 'dropdown' || editType === 'checkbox'" x-transition
                                                class="space-y-2">
                                                <label class="block text-xs font-medium text-gray-500">
                                                    <span
                                                        x-text="editType === 'checkbox' ? 'Opsi Multiple Choices' : 'Opsi Dropdown'"></span>
                                                </label>
                                                <template x-for="(opt, idx) in editOptions" :key="idx">
                                                    <div class="flex gap-2 items-center">
                                                        <input type="text" :name="'field_options[' + idx + ']'"
                                                            x-model="editOptions[idx]"
                        ...
                        <div class="border-t border-gray-100">
                            ...
                            <div x-show="showAddField" x-transition class="bg-gray-50 p-5 border-t border-gray-100">
                                <form action="{{ route('admin_inovasi.inovchalenge.tahap.fields.store', $tahap) }}"
                                    ...
                                    <div x-show="newFieldType === 'dropdown' || newFieldType === 'checkbox'" x-transition
                                        class="space-y-2">
                                        <label class="block text-sm font-medium text-gray-700">
                                            <span
                                                x-text="newFieldType === 'checkbox' ? 'Opsi Multiple Choices' : 'Opsi Dropdown'"></span>
                                        </label>
                                        <template x-for="(opt, idx) in newOptions" :key="idx">
                                            <div class="flex gap-2 items-center">
                                                <input type="text" :name="'field_options[' + idx + ']'"
                                                    x-model="newOptions[idx]"
                                                    class="flex-1 rounded-lg border-gray-300 shadow-sm focus:ring-teal-500 focus:border-teal-500 text-sm"
                ...
                @empty
                    <div class="bg-blue-50 border border-blue-200 rounded-2xl p-6 text-center text-sm text-blue-700">
                        ...
                        Belum ada seksi. Klik <strong>"Tambah Seksi"</strong> di atas untuk mulai mengelompokkan field ke
                        dalam beberapa bagian, atau tambahkan langsung sebagai <strong>Field Umum</strong> di bawah.
                    </div>
                @endforelse
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden"
                    ...
                    <div class="bg-gradient-to-r from-gray-600 to-gray-700 px-6 py-3 flex items-center justify-between">
                        <h3 class="text-white font-semibold text-[15px]">
                            <i class="fas fa-list-ul mr-1.5"></i> Field Umum <span
                                class="font-normal text-gray-300 text-xs">(tanpa seksi)</span>
                            <span class="ml-2 text-gray-400 text-xs">({{ $tahap->unsectionedFields->count() }}
                                field)</span>
                        </h3>
                    </div>
                        ...
                        <div class="bg-red-50 border-b border-red-200 px-6 py-3">
                            @foreach ($errors->all() as $error)
                                <p class="text-sm text-red-600"><i
                                        class="fas fa-exclamation-circle mr-1"></i>{{ $error }}</p>
                            @endforeach
                        </div>
                    @endif
                    ...
                    <div class="divide-y divide-gray-100">
                        @forelse($tahap->unsectionedFields as $field)
                            <div class="p-4 hover:bg-gray-50 transition group" x-data="{ editing: false, editOptions: {{ json_encode($field->field_options ?? []) }}, editType: '{{ $field->field_type }}' }">
                                ...
                                <div x-show="editing" x-transition>
                                    <form action="{{ route('admin_inovasi.inovchalenge.tahap.fields.update', $field) }}"
                                        method="POST" class="space-y-3">
                                        @csrf @method('PUT')
                                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
                                            <div>
                                            ...
                                            <div>
                                                ...
                                                <select name="is_required"
                                                    class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-teal-500 focus:border-teal-500 text-sm">
                                                    <option value="1" {{ $field->is_required ? 'selected' : '' }}>Ya
                                                    </option>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <select name="is_required"
                                            class="w-full rounded-lg border-gray-300 shadow-sm focus:ring-teal-500 focus:border-teal-500 text-sm">
                                            <option value="1"
                                                {{ old('is_required', '1') === '1' ? 'selected' : '' }}>Ya</option>
