                        ...
                        <td class="px-5 py-4 border-b border-gray-200 bg-white text-sm text-center">
                             @if ($submission->status == 'diajukan' && !$submission->is_confirmed)
                                 <div class="flex items-center justify-center gap-2">
                                     <a href="{{ route('equity_fakultas.employer-meetings.edit-draft', $submission->id) }}" class="text-blue-600 hover:text-blue-900 font-semibold" title="Edit">
                                         <i class='bx bx-edit text-xl'></i>
                                     </a>
                                     <form action="{{ route('equity_fakultas.employer-meetings.destroy', $submission->id) }}" method="POST" class="inline" onsubmit="return confirm('Yakin ingin menghapus proposal ini?')">
                                         @csrf
                                         @method('DELETE')
                                     ...
                                     <form action="{{ route('equity_fakultas.employer-meetings.confirm', $submission->id) }}" method="POST" class="inline" onsubmit="return confirm('Apakah Anda yakin ingin mengirim proposal ini ke admin? Setelah dikonfirmasi, Anda tidak dapat mengedit atau menghapus lagi.')">
                                         @csrf
