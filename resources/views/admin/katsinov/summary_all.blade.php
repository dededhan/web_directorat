...
<div class="table-data">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <p><strong>Status:</strong> 
                            @php
                                $avgScore = array_sum(array_values($overallAspectScores)) / count($overallAspectScores);
                            @endphp
                                ...
                                <table class="table table-sm">
                                    ...
                                    <tbody>
                                        ...
                                        @foreach($aspectNames as $key => $name)
                                            <tr>
                                                <td>
                                                    <span class="badge rounded-pill" style="background-color: {{ $aspectColors[$key] }}40; color: black; border: 1px solid {{ $aspectColors[$key] }}">
                                                        {{ $name }}
                                                    </span>
                                                </td>
                                                <td>{{ number_format($indicatorAspectScores[$indicator][$key], 1) }}%</td>
                                                <td>
                                                    @php
    ...
    <div class="card h-100 border-0 shadow-sm aspect-card">
        <div class="card-header d-flex align-items-center" style="background-color: {{ $aspectColors[$aspectKey] }}40; border-left: 5px solid {{ $aspectColors[$aspectKey] }};">
            <i class='bx bx-{{ $aspectKey == "technology" ? "chip" : 
                             ($aspectKey == "market" ? "store" : 
                             ($aspectKey == "organization" ? "building" : 
                             ($aspectKey == "manufacturing" ? "factory" : 
                             ($aspectKey == "partnership" ? "handshake" : 
                             ($aspectKey == "investment" ? "money" : 
            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="aspect-score-chart mb-3" style="height: 200px;">
                        <canvas id="chart_{{ $indicator }}_{{ $aspectKey }}"></canvas>
                    </div>
                    <div class="aspect-status text-center">
                        ...
                        <div class="mt-1">
                            <small>Overall: {{ number_format($indicatorAspectScores[$indicator][$aspectKey], 1) }}%</small>
                        </div>
                    </div>
                    ...
                    <div class="aspect-questions mt-3">
                        ...
                        <ul class="ps-3 mt-2">
                            @foreach($indicatorQuestions[$indicator][$aspectKey] as $qIndex => $question)
                                @php
                                    $score = $questionScores[$indicator][$aspectKey][$qIndex] ?? 0;
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                        ...
                        <p>The assessment of <strong>{{ $katsinov->title }}</strong> reveals the following insights:</p>
                        ...
                        <ul>
                            ...
                            <li>
                                ...
                                @if($earlyStageScore > $midStageScore && $earlyStageScore > $lateStageScore)
                                    This innovation is strongest in early-stage development (research and conceptualization)
                                @elseif($midStageScore > $earlyStageScore && $midStageScore > $lateStageScore)
                                    This innovation is strongest in mid-stage development (implementation and refinement)
                                @else
                                    This innovation is strongest in late-stage development (commercialization and market expansion)
                                @endif
                            </li>
                            ...
                            <li>
                                ...
                                @if($avgScore >= 80)
                                    This innovation shows strong overall readiness, suggesting it has a solid foundation across most aspects.
                                @elseif($avgScore >= 60)
                                    This innovation shows moderate readiness, with some aspects requiring improvement before proceeding.
                                @else
                                    This innovation requires significant development across multiple aspects before proceeding.
                                @endif
                            </li>
                        </ul>
...
#overallChart, #radarChart, [id^="indicator"], [id^="lineChart_"] {
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('Chart data ready:', { aspectLabels, aspectColors, aspectData });
    ...
    const questionCounts = {
        1: {
            'technology': 5, 'market': 3, 'organization': 4,
            'manufacturing': 3, 'partnership': 2, 'investment': 3, 'risk': 1
        },
        2: {
            'technology': 5, 'market': 3, 'organization': 4,
            'manufacturing': 2, 'partnership': 2, 'investment': 4, 'risk': 2
        },
        3: {
            'technology': 5, 'market': 3, 'organization': 3,
            'manufacturing': 3, 'partnership': 2, 'investment': 3, 'risk': 2
        },
        4: {
            'technology': 4, 'market': 4, 'organization': 3,
            'manufacturing': 4, 'partnership': 2, 'investment': 2, 'risk': 3
        },
        5: {
            'technology': 4, 'market': 4, 'organization': 4,
            'manufacturing': 4, 'partnership': 3, 'investment': 2, 'risk': 3
        },
        6: {
            'technology': 3, 'market': 4, 'organization': 2,
            'manufacturing': 1, 'partnership': 2, 'investment': 1, 'risk': 1
        }
    };
    ...
    for (let i = 1; i <= 6; i++) {
        aspects.forEach(aspect => {
            ...
            try {
                ...
                if (questionCount === 1) {
                    const score = (questionScores[i] && questionScores[i][aspect] && questionScores[i][aspect][0]) 
                                 ? questionScores[i][aspect][0] * 20 : 0;
                    
                    new Chart(chartCtx, {
                        type: 'bar',
                        data: {
                            labels: [`${aspect.charAt(0).toUpperCase() + aspect.slice(1)} Assessment`],
                            datasets: [{
                                label: 'Score',
                                data: [score],
                ...
                // Multiple questions case (use line chart)
                    ...
                    for (let q = 0; q < questionCount; q++) {
                        labels.push(`Q${q+1}`);
                        const score = (questionScores[i] && questionScores[i][aspect] && questionScores[i][aspect][q] !== undefined) 
