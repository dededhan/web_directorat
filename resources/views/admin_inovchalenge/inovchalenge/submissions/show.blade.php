        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <nav class="mb-6">
                <ol class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <li><i class="fas fa-chevron-right text-xs"></i></li>
                    <li><a href="{{ route('admin_inovchalenge.inovchalenge.submissions.index', $session) }}"
            ...
            @foreach ($submission->submissionTahap->sortBy(fn($st) => $st->tahap->tahap_ke) as $st)
                <div x-show="activeTab === {{ $st->tahap->tahap_ke }}" x-cloak class="space-y-6">
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        ...
                        <div class="p-5">
                            <form method="POST"
                                action="{{ route('admin_inovchalenge.inovchalenge.submissions.updateTahapStatus', $st) }}"
                                        ...
                                        <div class="ml-9 space-y-2">
                                            ...
                                            @if ($review->skor !== null)
                                                <div class="flex items-center gap-3 mb-2">
                                                    <div
                                                        class="w-10 h-10 rounded-xl flex items-center justify-center font-bold text-sm {{ $review->getSkorColor() }}">
                                                        {{ $review->skor }}
                                                    </div>
                                                    <div class="flex-1">
                                                        <div class="flex items-center justify-between mb-0.5">
                                                            <span class="text-[10px] font-semibold text-gray-500">Skor
                                                                Penilaian</span>
                                                            <span
                                                                class="text-[10px] text-gray-400">{{ $review->skor }}/100</span>
                                                        </div>
                                                        <div class="h-1.5 bg-gray-200 rounded-full overflow-hidden">
                                                            <div class="h-full rounded-full bg-gradient-to-r {{ $review->getSkorBarColor() }}"
                                                                style="width: {{ $review->skor }}%"></div>
                                                        </div>
                                                    </div>
                                                </div>
                                            @endif
                                            ...
                                            @if ($review->penilaian)
                                                <p class="text-sm text-gray-700"><strong
                                                        class="text-xs text-gray-400">Catatan
            ...
            <div class="mt-8 bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                ...
                <div class="p-6">
                    <form method="POST"
                        action="{{ route('admin_inovchalenge.inovchalenge.submissions.assignReviewer', [$session, $submission]) }}">
                        @csrf
                        @method('PATCH')
                        <div class="mb-4">
