<header class="flex h-16 flex-shrink-0 items-center justify-between bg-white px-6 shadow-md">
    <div class="flex items-center gap-3">
        ...
        <span class="hidden items-center rounded-full border border-amber-200 bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700 sm:inline-flex">
            <i class="fas fa-lightbulb mr-1.5"></i> Peserta Hackaton
        </span>
    </div>

    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open" type="button" class="flex items-center space-x-3 focus:outline-none">
            <span class="hidden font-medium text-gray-700 md:block">{{ auth()->user()->name ?? 'Peserta' }}</span>
        ...
        <div x-show="open" @click.away="open = false"
            ...
            <div class="border-b border-gray-100 px-4 py-2 text-xs text-gray-500">
                Masuk sebagai <strong class="text-gray-800">{{ auth()->user()->role ?? 'Peserta' }}</strong>
            </div>
            <a href="{{ route('hackaton.dashboard') }}"
                class="block px-4 py-2 text-sm text-gray-700 transition hover:bg-gray-100">
                <i class="fas fa-user-circle mr-2 text-amber-600"></i> Profil & Akun
            </a>
