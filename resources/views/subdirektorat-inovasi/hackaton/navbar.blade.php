<header class="flex min-h-20 flex-shrink-0 items-center justify-between border-b border-black bg-white px-5 sm:px-8 lg:px-12">
    ...
    <div class="relative" x-data="{ open: false }">
        <button @click="open = !open" type="button" class="flex items-center gap-3 text-left">
            <span class="hidden text-xs font-semibold uppercase tracking-[0.1em] md:block">{{ auth()->user()->name ?? 'Peserta' }}</span>
        ...
        <div x-show="open" @click.away="open = false"
            ...
            <div class="border-b border-black px-4 py-2 text-xs text-gray-500">
                Masuk sebagai <strong class="text-black">{{ auth()->user()->role ?? 'Peserta' }}</strong>
            </div>
