    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
            <div class="flex items-center gap-3">
                <a href="{{ route('hackaton.team.show', $submission) }}"
                ...
                <div>
                    <div class="flex items-center gap-2">
                        <span
                            class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-amber-100 text-amber-800 text-xs font-bold">
                            T{{ $tahap->tahap_ke }}
                        </span>
            ...
            <div class="flex flex-wrap gap-2">
                ...
                <span
                    ...
                    <i class="fas fa-lock mr-1 text-[10px]"></i> Mode Read-Only
                </span>
            </div>
        </div>
        ...
        <div class="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-xl flex items-center gap-3 text-amber-800">
            ...
            <div>
                <p class="font-semibold text-sm">Tampilan Anggota Tim (Read-Only)</p>
                <p class="text-xs mt-0.5 text-amber-700">Anda melihat formulir ini sebagai anggota tim yang berpartisipasi. Pengisian dan pembaruan data dilakukan oleh Ketua Tim.</p>
            </div>
        </div>
        ...
        @if ($submissionTahap->catatan_admin)
            <div class="mb-6 bg-orange-50 border border-orange-200 rounded-xl p-4 text-sm text-orange-700">
                <p class="font-semibold mb-1"><i class="fas fa-comment-alt mr-1"></i> Catatan dari Administrator:</p>
            ...
            @if ($hasSections && count($sectionList) > 1)
                <div class="hidden lg:block w-56 flex-shrink-0">
                    <div class="sticky top-8">
                        <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-3">Bagian Formulir</p>
                        <nav class="space-y-1">
                            @foreach ($sectionList as $i => $sec)
                                <a href="#{{ $sec['id'] }}"
                                    class="flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium text-gray-500 hover:text-amber-700 hover:bg-amber-50 transition group">
                                    <span
                                        class="w-5 h-5 rounded-md bg-gray-100 group-hover:bg-amber-100 flex items-center justify-center text-[10px] font-bold text-gray-400 group-hover:text-amber-700">{{ $i + 1 }}</span>
                        ...
                        <div class="mt-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                            <p class="text-[10px] text-gray-400 font-semibold uppercase">Total Pertanyaan</p>
                    ...
                    @foreach ($tahap->sections as $sIdx => $section)
                        <div id="section-{{ $section->id }}" class="mb-6 scroll-mt-8">
                            <div class="flex items-center gap-3 mb-4">
                                <div
                                    class="w-8 h-8 rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white text-xs font-bold shadow-sm">
                                    {{ $sIdx + 1 }}
                                </div>
                                <div>
                            ...
                            <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                                <div class="divide-y divide-gray-50">
                                    @if ($section->fields->isEmpty())
                                        <div class="p-6 text-center text-sm text-gray-400">Belum ada kolom pertanyaan di bagian ini.</div>
                                    @else
                ...
                @else
                    <div class="mb-6">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <h2 class="text-base font-bold text-gray-900">Formulir Tahap {{ $tahap->tahap_ke }}</h2>
                        </div>
                        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                            @if ($tahap->fields->isEmpty())
                                <div class="p-6 text-center text-sm text-gray-400">Belum ada field yang dikonfigurasi untuk tahap ini.</div>
                            @else
                                <div class="divide-y divide-gray-50">
