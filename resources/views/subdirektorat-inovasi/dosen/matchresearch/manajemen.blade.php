                            ...
                            <div class="w-full overflow-visible">
                                <table class="w-full divide-y divide-gray-200">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            <th scope="col"
                                                class="px-4 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider w-6/12">
                                            ...
                                            <th scope="col"
                                                class="px-4 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider w-3/12">
                                            ...
                                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                                <td class="px-4 py-5">
                                                    <div class="flex items-start space-x-3">
                                                        <div class="flex-shrink-0">
                                                            <div
                                                                class="w-10 h-10 bg-gradient-to-br from-teal-100 to-teal-200 rounded-xl flex items-center justify-center">
                                                        ...
                                                        <div class="min-w-0 flex-1">
                                                            ...
                                                            <p
                                                                class="text-xs lg:text-sm text-gray-500 mt-1 flex items-center">
                                                ...
                                                <td class="px-4 py-5 text-center">
                                                    <span
                                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold {{ getStatusColor($submission->status) }}">
                                                        <i
                                                            class='bx {{ getStatusIcon($submission->status) }} mr-1 text-xs'></i>
                                                ...
                                                <td class="px-4 py-5 text-center">
                                                    <div class="flex items-center justify-center space-x-2">
                                                        ...
                                                        @elseif(in_array($submission->status, ['diterima', 'draft_laporan', 'revisi' ]))
                                                            @php
                                                                $buttonText = '';
                                                                $buttonClass = '';
                                                                if ($submission->status === 'diterima') {
                                                                    $buttonText = 'Lengkapi Laporan';
                                                                    $buttonClass = 'bg-teal-600 hover:bg-teal-700';
                                                                } elseif ($submission->status === 'draft_laporan') {
                                                                    $buttonText = 'Lanjutkan Laporan';
                                                                    $buttonClass = 'bg-amber-500 hover:bg-amber-600';
                                                                } else {
                                                                    $buttonText = 'Revisi Laporan';
                                                                    $buttonClass = 'bg-orange-500 hover:bg-orange-600';
                                                                }
                                                            @endphp
                                                            <a href="{{ route('subdirektorat-inovasi.dosen.matchresearch.report.form', $submission->id) }}"
                                                                class="inline-flex items-center px-3 py-2 {{ $buttonClass }} text-white rounded-xl transition-all duration-200 font-semibold text-sm shadow-md hover:shadow-lg transform hover:scale-105"
                                                                title="{{ $buttonText }}">
                                                        ...
                                                        @if (
                                                            ($submission->status == 'ditolak_awal' || $submission->status == 'revisi' || $submission->status == 'tolak') &&
                                                                $submission->rejection_note)
                                                            <button
                                                                @click="openNoteId = (openNoteId === {{ $submission->id }} ? null : {{ $submission->id }})"
                                                        ...
                                                        <a href="{{ route('subdirektorat-inovasi.dosen.matchresearch.logbook', $submission->id) }}"
                                                            class="inline-flex items-center px-3 py-2 bg-gray-100 text-gray-800 rounded-xl hover:bg-gray-200 transition-all duration-200 font-medium text-sm border-2 border-gray-200 hover:border-gray-300"
                                                            title="Lihat Logbook">
                                                            <i class='bx bx-history mr-1 text-sm'></i>
                                                            Logbook
                                                        </a>
                                                    </div>
                                                </td>
                                            ...
                                            @if (
                                                ...
                                                <tr x-show="openNoteId === {{ $submission->id }}"
                                                    ...
                                                    <td colspan="3" class="px-4 py-4">
                                                        <div
                                                            class="bg-red-50 border-l-4 border-red-400 rounded-xl p-4 mx-2">
                                                            <div class="flex items-start">
                                                                <i
                                                                    class='bx bxs-info-circle text-red-500 text-lg mr-3 mt-0.5'></i>
                                                                <div>
                                                                    <h4 class="font-bold text-red-800 mb-2">Catatan dari
                                                                        Admin:</h4>
                                                                    <p class="text-red-700 text-sm leading-relaxed">
                                                                        {{ $submission->rejection_note }}</p>
                                                                </div>
                                                            </div>
                                                        </div>
                                                    </td>
                                ...
                                <div
                                    ...
                                    <div class="flex items-start justify-between mb-3">
                                        <div class="flex items-start space-x-3 flex-1 min-w-0">
                                            <div class="flex-shrink-0">
                                                <div
                                                    class="w-8 h-8 bg-gradient-to-br from-teal-100 to-teal-200 rounded-lg flex items-center justify-center">
                                        ...
                                        <div class="flex-shrink-0 ml-2">
                                            <span
                                                class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium {{ getStatusColor($submission->status) }}">
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        @elseif(in_array($submission->status, ['diterima', 'draft_laporan', 'revisi']))
                                            @php
                                                ...
                                                $buttonClassMobile = '';
                                                if ($submission->status === 'diterima') {
                                                    $buttonTextMobile = 'Lengkapi Laporan';
                                                    $buttonClassMobile = 'bg-teal-600 hover:bg-teal-700';
                                                } elseif ($submission->status === 'draft_laporan') {
                                                    $buttonTextMobile = 'Lanjutkan Laporan';
                                                    $buttonClassMobile = 'bg-amber-500 hover:bg-amber-600';
                                                } else { // revisi
                                                    $buttonTextMobile = 'Revisi Laporan';
                                                    $buttonClassMobile = 'bg-orange-500 hover:bg-orange-600';
                                                }
                                            @endphp
                                            <a href="{{ route('subdirektorat-inovasi.dosen.matchresearch.report.form', $submission->id) }}"
                                                class="w-full flex items-center justify-center px-4 py-2 {{ $buttonClassMobile }} text-white rounded-xl text-sm font-semibold transition-all">
                                            ...
                                            <div x-data="{ showNote: false }" class="w-full">
                                                ...
                                                <div x-show="showNote"
                                                    ...
                                                    <div class="flex items-start">
                                                        ...
                                                        <div>
                                                            <h5 class="font-semibold text-red-800 text-sm mb-1">Catatan
                                                                Admin:</h5>
                    ...
                    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                        <div class="text-center py-20 px-6">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500 mb-8 max-w-md">Mulailah dengan mengajukan proposal matchmaking
                                    research untuk memulai kolaborasi penelitian.</p>
                                <a href="{{ route('subdirektorat-inovasi.dosen.matchresearch.list-sesi') }}"
                                    class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-semibold rounded-xl hover:from-teal-600 hover:to-teal-700 transform hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg">
                                    <i class='bx bx-plus-circle mr-2 text-lg'></i>
                                    Usulkan Proposal Baru
