                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($katsinov->status === 'draft')
                                        <a href="{{ route('subdirektorat-inovasi.dosen.katsinov-v2.edit', $katsinov->id) }}" 
                                    ...
                                    @if(Auth::user()->role === 'admin_inovasi' && 
                                        ...
                                        <a href="{{ route('subdirektorat-inovasi.dosen.katsinov-v2.full-report', $katsinov->id) }}" 
                                    ...
                                    @if($katsinov->status === 'completed')
                                        <a href="{{ route('subdirektorat-inovasi.dosen.katsinov-v2.certificate', $katsinov->id) }}" 
                                           target="_blank"
                                        ...
                                        {{-- Download Report Pengukuran button --}}
                                        <a href="{{ route('subdirektorat-inovasi.dosen.katsinov-v2.download-report', $katsinov->id) }}" 
                                           class="text-teal-600 hover:text-teal-900" title="Download Report Pengukuran">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
document.getElementById('assignForm').addEventListener('submit', function(e) {
    ...
    fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.index') }}/${katsinovId}/assign-reviewer`, {
        method: 'POST',
        headers: {
...
function startReview(katsinovId) {
    if (confirm('Mulai review untuk katsinov ini?')) {
        fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.index') }}/${katsinovId}/start-review`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
document.getElementById('statusForm').addEventListener('submit', function(e) {
    ...
    if (confirm(`Are you sure you want to change status to ${newStatus.toUpperCase()}?`)) {
        fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.index') }}/${katsinovId}/change-status`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
...
function viewReview(katsinovId) {
    fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.index') }}/${katsinovId}/review`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
...
function deleteKatsinov(katsinovId) {
    if (confirm('Apakah Anda yakin ingin menghapus proposal ini? Data tidak dapat dikembalikan!')) {
        fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.index') }}/${katsinovId}`, {
            method: 'DELETE',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
