    ...
    <div class="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-lg shadow-lg p-8 mb-6 border border-blue-200">
        <div class="flex items-center justify-between">
            ...
            <div class="flex gap-3">
                ...
                @endif
                <a href="{{ route('subdirektorat-inovasi.dosen.katsinov-v2.summary', $katsinov->id) }}" 
...
function startReview() {
    if (confirm('Mulai review untuk katsinov ini?')) {
        // This function should not be called in dosen view
        // Only admin_inovasi and validator can start review
        alert('Anda tidak memiliki akses untuk memulai review');
    }
}

function submitForReview() {
    if (confirm('Submit data ini untuk review?\n\nPastikan Anda telah melengkapi:\n1. Form Judul Inovasi\n2. Form Lampiran\n3. Form Informasi Dasar')) {
        fetch(`{{ route('subdirektorat-inovasi.dosen.katsinov-v2.submit', $katsinov->id) }}`, {
            method: 'POST',
            headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert(data.message || 'Gagal submit katsinov');
            }
        })
        .catch(error => {
...
// Complete form functionality removed for dosen
