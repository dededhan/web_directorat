        ...
        <header class="mb-10">
            <nav class="text-sm text-gray-500 mb-3" aria-label="Breadcrumb">
                <ol class="list-none p-0 inline-flex items-center space-x-2">
                    ...
                    <li><a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}" class="hover:text-teal-600 transition-colors duration-200">Hibah Modul Ajar</a></li>
                    <li><i class='bx bx-chevron-right text-base text-gray-400'></i></li>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl lg:text-3xl font-bold text-gray-800">Manajemen Proposal Hibah Modul Ajar</h1>
                    <p class="mt-2 text-gray-600 text-base">Kelola semua proposal hibah modul ajar yang telah Anda ajukan.</p>
                </div>
                <div class="flex-shrink-0">
                    <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}" class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-semibold rounded-xl hover:from-teal-600 hover:to-teal-700 transform hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg">
            ...
            <div class="bg-gradient-to-r from-teal-500 to-teal-600 px-6 lg:px-8 py-6">
                <div class="flex items-center justify-between text-white">
                    <h2 class="text-xl lg:text-2xl font-bold flex items-center">
                        ...
                        Daftar Proposal Hibah Modul Ajar
                    </h2>
                ...
                <div class="w-full overflow-visible">
                    <table class="w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th scope="col" class="px-6 py-4 text-left text-xs font-bold text-gray-600 uppercase tracking-wider w-5/12">
                                    <div class="flex items-center space-x-1">
                                        <i class='bx bx-book-content text-base text-blue-500'></i>
                                        <span>Judul Modul & Sesi</span>
                                    </div>
                                </th>
                                ...
                                <th scope="col" class="px-6 py-4 text-center text-xs font-bold text-gray-600 uppercase tracking-wider w-3/12">
                                    <div class="flex items-center justify-center space-x-1">
                                ...
                                <div class="flex items-start space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <p class="font-semibold text-gray-900 text-sm lg:text-base leading-relaxed break-words">
                                            {{ $proposal->judul_modul ?? 'Belum ada judul (Draft)' }}
                                        </p>
                                        ...
                                        <p class="text-xs text-gray-500 mt-1 line-clamp-2">{{ Str::limit($proposal->ringkasan_modul, 80) }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                                ...
                                <div x-data="{ open: false }" class="relative inline-block text-left">
                                    ...
                                    <div x-show="open" 
                                        ...
                                        <div class="py-1" role="menu">
                                            <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.show', $proposal->id) }}" class="flex items-center w-full px-4 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors">
                                                <i class='bx bx-show-alt mr-3 text-lg text-blue-500'></i>Lihat Detail
                                            </a>
                                            
                                            @if(in_array($proposal->status, ['draft', 'diajukan']))
                                            <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.edit', $proposal->id) }}" class="flex items-center w-full px-4 py-3 text-sm text-gray-700 hover:bg-yellow-50 hover:text-yellow-700 transition-colors">
                                                <i class='bx bx-edit-alt mr-3 text-lg text-yellow-600'></i>Edit Proposal
                                            </a>
                                            @endif

                                            @if($proposal->status === 'draft')
                                            <form action="{{ route('subdirektorat-inovasi.dosen.hibah_modul.confirm', $proposal->id) }}" method="POST" class="w-full">
                                                @csrf
                                            ...
                                            <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.laporanAkhir', $proposal->id) }}" class="flex items-center w-full px-4 py-3 text-sm text-gray-700 hover:bg-purple-50 hover:text-purple-700 transition-colors">
                                            ...
                                            <form action="{{ route('subdirektorat-inovasi.dosen.hibah_modul.destroy', $proposal->id) }}" method="POST" onsubmit="return confirm('Yakin hapus proposal ini?')" class="w-full">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-gray-500 mb-8 max-w-md">Mulai buat proposal hibah modul ajar Anda untuk mendapatkan pendanaan.</p>
                                    <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}" class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-semibold rounded-xl hover:from-teal-600 hover:to-teal-700 transform hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg">
                                        <i class='bx bx-plus-circle mr-2 text-lg'></i>
                                        Usulkan Proposal Baru
                                    </a>
