...
@section('content')
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100" x-data="proposalFormData()">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <header class="mb-8">
                <nav class="text-sm text-gray-500 mb-3" aria-label="Breadcrumb">
                    <ol class="list-none p-0 inline-flex items-center space-x-2">
                        <li><a href="{{ route('subdirektorat-inovasi.dosen.dashboard') }}"
                                class="hover:text-teal-600">Home</a></li>
                        <li><i class='bx bx-chevron-right text-base text-gray-400'></i></li>
                        <li><a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.manage') }}"
                                class="hover:text-teal-600">Hibah Modul Ajar</a></li>
                        <li><i class='bx bx-chevron-right text-base text-gray-400'></i></li>
                        <li><a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}"
                                class="hover:text-teal-600">Sesi</a></li>
                        <li><i class='bx bx-chevron-right text-base text-gray-400'></i></li>
                ...
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="mt-2 text-gray-600">Sesi: <span
                                class="font-semibold text-teal-600">{{ $sesi->nama_sesi }}</span></p>
                    </div>
                    <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}"
                        class="inline-flex items-center px-4 py-2.5 bg-white border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 shadow-sm">
            ...
            <form action="{{ route('subdirektorat-inovasi.dosen.hibah_modul.store', $sesi->id) }}" method="POST"
                enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                @csrf
                <div class="bg-gradient-to-r from-teal-500 to-teal-600 px-6 lg:px-8 py-5">
                    <h2 class="text-xl lg:text-2xl font-bold text-white flex items-center"><i
                            class='bx bx-edit mr-3 text-2xl'></i> Formulir Proposal</h2>
                </div>

                <div class="p-6 lg:p-8 space-y-8">
                    {{-- Informasi Modul Section --}}
                    <div class="space-y-6">
                        <h3 class="text-lg font-bold text-gray-800 flex items-center border-b pb-3"><i
                                class='bx bx-book-content text-teal-600 mr-2 text-xl'></i> Informasi Modul</h3>
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Judul Modul <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="judul_modul" value="{{ old('judul_modul') }}" required
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Ringkasan Modul <span
                                    class="text-red-500">*</span></label>
                            <textarea name="ringkasan_modul" rows="6" required
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                placeholder="Tuliskan ringkasan modul ajar Anda (maksimal 300 kata)">{{ old('ringkasan_modul') }}</textarea>
                        </div>

                        {{-- === INPUT KATA KUNCI BARU (TAGIFY) === --}}
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Kata Kunci</label>
                            <input name="kata_kunci" x-ref="keywords" value="{{ old('kata_kunci') }}" class="w-full"
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <div class="w-full bg-white border-2 border-gray-300 rounded-xl p-2 min-h-[50px] flex flex-wrap gap-2 items-center cursor-pointer"
                                    ...
                                    <span x-show="selectedSdgsFokus.length === 0" class="text-gray-400 text-sm px-2">Pilih
                                        SDG Fokus...</span>
                                </div>
                            ...
                            <div class="relative">
                                <div class="w-full bg-white border-2 border-gray-300 rounded-xl p-2 min-h-[50px] flex flex-wrap gap-2 items-center cursor-pointer"
                                    ...
                                    <template x-for="(sdg, index) in selectedSdgsPendukung" :key="index">
                                        <span
                                            class="flex items-center gap-1.5 bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-1.5 rounded-md">
                                            <span x-text="sdg"></span>
                                            <button type="button" @click.stop="removeSdgPendukung(index)"
                                                class="text-blue-600 hover:text-blue-800 font-bold">&times;</button>
                                        </span>
                                    </template>
                                    <span x-show="selectedSdgsPendukung.length === 0"
                                        class="text-gray-400 text-sm px-2">Pilih SDG Pendukung...</span>
                                </div>
                                <div x-show="sdgPendukungDropdownOpen" @click.away="sdgPendukungDropdownOpen = false"
                                    x-transition
                                    class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-xl py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm"
                                    style="display: none;">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Tahun Usulan <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="tahun_usulan" value="{{ old('tahun_usulan') }}" required
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                    placeholder="Contoh: 2025">
                            </div>
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Tahun Pelaksanaan <span
                                        class="text-red-500">*</span></label>
                                <input type="text" name="tahun_pelaksanaan" value="{{ old('tahun_pelaksanaan') }}"
                                    required
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                    placeholder="Contoh: 2025">
                            </div>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Tempat Pelaksanaan <span
                                    class="text-red-500">*</span></label>
                        ...
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Anggaran Usulan <span
                                    class="text-red-500">*</span></label>
                            <div class="mb-2 p-3 bg-teal-50 border border-teal-200 rounded-lg">
                                <p class="text-sm text-teal-800 font-semibold">
                                    <i class='bx bx-info-circle mr-1'></i>Dana Maksimal:
                                    <span class="text-teal-900">Rp
                                        {{ number_format($sesi->nominal_usulan ?? 0, 0, ',', '.') }}</span>
                                </p>
                            </div>
                            <div class="relative">
                                <span
                                    class="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500 font-semibold">Rp</span>
                                <input type="text" name="anggaran_usulan" value="{{ old('anggaran_usulan') }}"
                                    required
                                    class="w-full pl-12 pr-4 py-3 border-2 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                    :class="anggaranError || $errors->has('anggaran_usulan') ? 'border-red-500 bg-red-50' : 'border-gray-300'"
                                    placeholder="0" x-mask:dynamic="$money($input, ',', '.')" x-model="anggaranUsulan"
                                    @input="validateAnggaran">
                            </div>
                            <p class="text-xs text-gray-500 mt-2"
                                x-show="!anggaranError && !$errors->has('anggaran_usulan')">
                                <i class='bx bx-info-circle mr-1'></i>Masukkan anggaran dalam Rupiah
                            </p>
                            <p class="text-xs text-red-600 mt-2 font-semibold" x-show="anggaranError"
                                x-text="anggaranError"></p>
                            @error('anggaran_usulan')
                                <p class="text-xs text-red-600 mt-2 font-semibold">{{ $message }}</p>
                            @enderror
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Platform Digital yang Digunakan
                                <span class="text-red-500">*</span></label>
                            <input type="text" name="platform_digital" value="{{ old('platform_digital') }}" required
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                        ...
                        <div>
                            ...
                            <input type="text" name="mitra" value="{{ old('mitra') }}"
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Modul Interdisiplin Digital
                                    <span class="text-red-500">*</span></label>
                                <select name="modul_interdisiplin" required
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all">
                                    <option value="">-- Pilih --</option>
                                    <option value="Ada" {{ old('modul_interdisiplin') == 'Ada' ? 'selected' : '' }}>Ada
                                    </option>
                                    <option value="Draft" {{ old('modul_interdisiplin') == 'Draft' ? 'selected' : '' }}>
                                        Draft</option>
                                </select>
                            </div>

                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">Publikasi Media Massa <span
                                        class="text-red-500">*</span></label>
                                <select name="publikasi_media_massa" required
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                    x-model="publikasiMediaMassa">
                                    <option value="">-- Pilih --</option>
                                    <option value="Ada" {{ old('publikasi_media_massa') == 'Ada' ? 'selected' : '' }}>
                                        Ada</option>
                                    <option value="Draft"
                                        {{ old('publikasi_media_massa') == 'Draft' ? 'selected' : '' }}>Draft</option>
                                </select>
                            </div>

                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">HKI <span
                                        class="text-red-500">*</span></label>
                                <select name="hki" required
                                    class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                    x-model="hkiStatus">
                                    <option value="">-- Pilih --</option>
                        ...
                        <div x-show="publikasiMediaMassa === 'Ada' || publikasiMediaMassa === 'Draft'" x-transition>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Nama Media Massa <span
                                    class="text-red-500">*</span></label>
                            <input type="text" name="nama_media_massa" value="{{ old('nama_media_massa') }}"
                        ...
                        <div x-show="hkiStatus === 'Ada' || hkiStatus === 'Draft'" x-transition>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis HKI dan Judul <span
                                    class="text-red-500">*</span></label>
                            <textarea name="jenis_hki_dan_judul" rows="3" :required="hkiStatus === 'Ada' || hkiStatus === 'Draft'"
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 transition-all"
                                placeholder="Contoh: Hak Cipta - Modul Pembelajaran Inovatif">{{ old('jenis_hki_dan_judul') }}</textarea>
                        </div>

                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">File Proposal (PDF) <span
                                    class="text-red-500">*</span></label>
                            <input type="file" name="file_proposal" accept=".pdf" required
                                class="w-full px-4 py-3 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 focus:border-teal-500 file:mr-4 file:py-2 file:px-4 file:rounded-lg file:border-0 file:text-sm file:font-semibold file:bg-teal-50 file:text-teal-700 hover:file:bg-teal-100">
                            <p class="text-xs text-gray-500 mt-2"><i class='bx bx-info-circle mr-1'></i>Maksimal 10MB,
                                format PDF</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="flex items-center justify-between border-b pb-3">
                            <h3 class="text-lg font-bold text-gray-800 flex items-center"><i
                                    class='bx bx-group text-teal-600 mr-2 text-xl'></i> Anggota Penyusun</h3>
                            <button type="button" @click="addAnggota"
                                class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 transition-all shadow-sm"><i
                                    class='bx bx-plus mr-2'></i>Tambah Anggota</button>
                        </div>

                        <div class="space-y-6">
                            <template x-for="(item, index) in anggota" :key="item.id">
                                <div
                                    class="bg-white rounded-2xl shadow-lg border border-gray-100 relative animate-fade-in">
                                    <button type="button" @click="removeAnggota(index)"
                                        class="absolute top-4 right-4 z-10 w-10 h-10 bg-red-50 text-red-500 rounded-xl hover:bg-red-100 flex items-center justify-center"><i
                                    ...
                                    <div
                                        ...
                                        <h3 class="text-lg font-bold text-blue-800 flex items-center"
                                            x-text="'Anggota ' + (index + 1)"></h3>
                                    </div>
                                    <div class="p-6 space-y-4" x-init="initAnggotaSearch(item.id, index)">
                                        <div>
                                            <label :for="'anggota-search-' + item.id"
                                                class="block text-sm font-semibold text-gray-700 mb-2">Cari Nama/NIP
                                                Dosen</label>
                                            <select :id="'anggota-search-' + item.id"
                                                placeholder="Ketik untuk mencari dosen..."></select>

                                            {{-- Hidden inputs to store data --}}
                                            <input type="hidden" :name="`anggota[${index}][nama_dosen]`"
                                                x-model="item.nama_dosen">
                                            <input type="hidden" :name="`anggota[${index}][nip]`" x-model="item.nip">
                                            <input type="hidden" :name="`anggota[${index}][fakultas]`"
                                                x-model="item.fakultas">
                                            <input type="hidden" :name="`anggota[${index}][prodi]`"
                                                x-model="item.prodi">
                                        </div>
                            ...
                            <p x-show="anggota.length === 0" class="text-center text-gray-500 py-4">Belum ada anggota yang
                                ditambahkan.</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="px-6 lg:px-8 py-5 bg-gray-50 border-t-2 border-gray-100">
                    <div class="flex flex-col sm:flex-row items-center justify-end gap-3">
                        <a href="{{ route('subdirektorat-inovasi.dosen.hibah_modul.sesi') }}"
                            class="w-full sm:w-auto px-6 py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded-xl hover:bg-gray-100 text-center"><i
                                class='bx bx-x mr-2'></i>Batal</a>
                        <button type="submit" :disabled="anggaranError || $errors->has('anggaran_usulan')"
                            class="w-full sm:w-auto px-6 py-3 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-semibold rounded-xl hover:from-teal-600 hover:to-teal-700 transform hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none"><i
                                class='bx bx-save mr-2'></i>Simpan Proposal</button>
                    </div>
                </div>
            </form>
        </div>
    ...
    <script>
        function proposalFormData() {
            return {
                ...
                get availableSdgsFokus() {
                    return this.sdgFokusOptions.filter(opt => !this.selectedSdgsFokus.includes(opt));
                },
                selectSdgFokus(sdg) {
                ...
                get availableSdgsPendukung() {
                    return this.sdgPendukungOptions.filter(opt => !this.selectedSdgsPendukung.includes(opt));
                },
                ...
                // --- Data untuk Anggaran Usulan ---
                nominalUsulanMax: {{ $sesi->nominal_usulan ?? 0 }},
                anggaranUsulan: '{{ old('anggaran_usulan', '') }}',
                anggaranError: null,
                ...
                init() {
                    ...
                    // Validasi anggaran jika ada old input
                    if (this.anggaranUsulan) {
                        this.$nextTick(() => {
                            this.validateAnggaran();
                        });
                    }
                ...
                validateAnggaran() {
                    // Remove formatting (commas and dots)
                    const cleanValue = this.anggaranUsulan.replace(/[^0-9]/g, '');
                    const numericValue = cleanValue ? parseInt(cleanValue) : 0;

                    if (numericValue > this.nominalUsulanMax) {
                        this.anggaranError = 'Anggaran usulan tidak boleh melebihi dana maksimal (Rp ' + this
                            .formatCurrency(this.nominalUsulanMax) + ')';
                    } else {
                        this.anggaranError = null;
                    }
                },
                            ...
                            onChange: (value) => {
                                ...
                                if (this.anggota[index]) {
                                    if (selectedData) {
                                        ...
                                        this.anggota[index].nama_dosen = '';
                                        this.anggota[index].nip = '';
                                        this.anggota[index].fakultas = '';
                                        this.anggota[index].prodi = '';
