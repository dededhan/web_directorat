...
@section('content')
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100" x-data="{ anggota: [{ nama_dosen: '', nip: '', fakultas: '', prodi: '' }] }">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">

        <header class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Buat Proposal Modul Ajar</h1>
            <p class="mt-2 text-gray-600">Sesi: {{ $sesi->nama_sesi }}</p>
        </header>
        ...
        <form action="{{ route('subdirektorat-inovasi.dosen.hibah_modul.store', $sesi->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-lg p-8 space-y-6">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Judul Modul <span class="text-red-500">*</span></label>
                <input type="text" name="judul_modul" value="{{ old('judul_modul') }}" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">
            </div>
            ...
            <div>
                ...
                <textarea name="ringkasan_modul" rows="5" required class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500">{{ old('ringkasan_modul') }}</textarea>
                <p class="text-sm text-gray-500 mt-1">Maksimal 300 kata</p>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">Kata Kunci</label>
                <input type="text" name="kata_kunci" id="kata_kunci" value="{{ old('kata_kunci') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                <p class="text-sm text-gray-500 mt-1">Pisahkan dengan koma, misal: pembelajaran, inovatif, digital</p>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">SDGs Terkait</label>
                <input type="text" name="sdgs" id="sdgs" value="{{ old('sdgs') }}" class="w-full px-4 py-3 border border-gray-300 rounded-lg">
                <p class="text-sm text-gray-500 mt-1">Pisahkan dengan koma, misal: 4, 8, 17</p>
            </div>

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-2">File Proposal (PDF) <span class="text-red-500">*</span></label>
                <input type="file" name="file_proposal" accept=".pdf" required class="w-full px-4 py-3 border border-gray-300 rounded-lg">
            ...
            <div class="flex items-center justify-end space-x-4 pt-6 border-t">
                ...
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-teal-500 to-teal-600 text-white font-semibold rounded-lg hover:from-teal-600 hover:to-teal-700 shadow-md">
                    <i class='bx bx-save mr-2'></i>Simpan Proposal
                </button>
            </div>
        </form>
    </div>
