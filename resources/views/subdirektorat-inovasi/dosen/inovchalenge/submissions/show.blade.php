...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @endphp
        <div class="bg-gradient-to-r {{ $overallColors[$statusKey] ?? 'from-gray-400 to-gray-500' }} rounded-2xl p-5 mb-8 text-white shadow-lg">
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div class="flex flex-col gap-2">
                            <div class="flex items-center gap-2">
                                ...
                                <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full bg-{{ $s['color'] }}-100 text-{{ $s['color'] }}-700">
                                    <i class="fas {{ $s['icon'] }} mr-1 text-[10px]"></i>{{ $s['label'] }}
                                </span>
                            </div>
                            <div class="flex items-center gap-2">
                                <span class="text-xs text-gray-400 w-16">Admin:</span>
                                <span class="inline-flex items-center px-2 py-0.5 text-xs font-semibold rounded-full bg-{{ $a['color'] }}-100 text-{{ $a['color'] }}-700">
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100">
                <h2 class="text-lg font-bold text-gray-900"><i class="fas fa-users mr-2 text-teal-500"></i>Anggota Tim</h2>
            </div>
            <div class="p-6">
                    ...
                    <div class="divide-y divide-gray-100">
                        @foreach($submission->members as $member)
                            <div class="flex items-center justify-between py-3 {{ $loop->first ? '' : '' }}">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-teal-400 to-teal-600 flex items-center justify-center text-white font-bold text-sm">
                                        {{ strtoupper(substr($member->nama_lengkap, 0, 1)) }}
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-gray-900">{{ $member->nama_lengkap }}</p>
                                        <p class="text-xs text-gray-400">{{ ucfirst($member->tipe_anggota) }}{{ $member->peran ? ' — '.$member->peran : '' }}</p>
                                    </div>
                                </div>
                                <div>
                                    @if($member->tipe_anggota === 'alumni')
                                        ...
                                        @endphp
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $apColors[$member->approval_status] ?? 'bg-gray-100 text-gray-700' }}">
