        ...
        <form id="tahapForm"
                ...
                <div class="p-6 space-y-6">
                    @foreach($tahap->fields->sortBy('urutan') as $field)
                        @php
                            ...
                            $currentValue = null;
                            if ($fv) {
                                if ($field->field_type === 'file') $currentValue = $fv->value_file_path;
                                elseif ($field->field_type === 'url') $currentValue = $fv->value_url;
                                else $currentValue = $fv->value_text;
                            }
                        @endphp
                        <div>
            ...
            @if($isEditable && $tahap->fields->isNotEmpty())
                <div class="flex flex-col sm:flex-row gap-3 mb-8">
                    <button type="submit" class="flex-1 inline-flex items-center justify-center px-6 py-3 bg-gradient-to-r from-gray-500 to-gray-600 text-white font-semibold text-sm rounded-xl hover:from-gray-600 hover:to-gray-700 transition shadow">
                ...
                <div class="p-6">
                        ...
                        @forelse($members as $member)
                            <div class="flex items-center justify-between py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-400 to-indigo-600 flex items-center justify-center text-white font-bold text-sm">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400">
                                            ...
                                            @if($member->nik_nim_nip) · {{ $member->nik_nim_nip }} @endif
                                            @if($member->institusi_fakultas) · {{ $member->institusi_fakultas }} @endif
                                        </p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-2">
                                    @if($member->peran === 'Ketua')
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold bg-indigo-100 text-indigo-700">
                                    ...
                                    @else
                                        @if($member->tipe_anggota === 'alumni')
                                            ...
                                            @endphp
                                            <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold {{ $apColors[$member->approval_status] ?? 'bg-gray-100 text-gray-600' }}">
                    ...
                    @if($isEditable)
                        <div class="border-t border-gray-100 pt-6">
                            <h3 class="text-sm font-bold text-gray-700 mb-4"><i class="fas fa-user-plus mr-1.5 text-indigo-500"></i>Tambah Anggota</h3>
                            <form method="POST" action="{{ route('subdirektorat-inovasi.dosen.inovchalenge.members.store', $submission) }}">
                                @csrf
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 mb-1">Tipe Anggota <span class="text-red-500">*</span></label>
                                    ...
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-600 mb-1">Institusi/Fakultas</label>
