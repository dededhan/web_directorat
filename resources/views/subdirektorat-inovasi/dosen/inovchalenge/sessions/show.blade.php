    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <nav class="mb-6">
            <ol class="flex items-center space-x-2 text-sm text-gray-500">
                <li><a href="{{ route('subdirektorat-inovasi.dosen.inovchalenge.sessions.index') }}" class="hover:text-teal-600">Innovation Challenge</a></li>
            ...
            <div class="p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 text-sm">
                    <div>
                        ...
                        <dd class="mt-1 text-gray-700">{{ $session->periode_awal->format('d M Y') }} — {{ $session->periode_akhir->format('d M Y') }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-400 uppercase">Dana Maksimal</dt>
                        <dd class="mt-1 text-gray-700">{{ $session->dana_maksimal ? 'Rp ' . number_format($session->dana_maksimal, 0, ',', '.') : '-' }}</dd>
                    </div>
                    <div>
                        <dt class="text-xs font-medium text-gray-400 uppercase">Anggota Tim</dt>
                        <dd class="mt-1 text-gray-700">{{ $session->min_anggota ?? '?' }} — {{ $session->max_anggota ?? '?' }} orang</dd>
                    </div>
                </div>
            </div>
        </div>
            ...
            @foreach($session->tahap as $tahap)
                <div class="bg-white rounded-xl shadow border border-gray-100 p-5">
                    <div class="flex items-center mb-2">
                        <span class="flex items-center justify-center w-8 h-8 rounded-lg text-white text-sm font-bold
