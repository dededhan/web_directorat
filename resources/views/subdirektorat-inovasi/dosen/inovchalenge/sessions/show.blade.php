...
@section('content')
    @php
        $now = now();
        $totalDays = max($session->periode_awal->diffInDays($session->periode_akhir), 1);
        $elapsed = max($session->periode_awal->diffInDays($now), 0);
        $progress = min(round(($elapsed / $totalDays) * 100), 100);
        $daysLeft = max($now->diffInDays($session->periode_akhir, false), 0);
    ...
    <div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {{-- Hero Card --}}
            <div class="relative bg-gradient-to-br from-teal-600 via-teal-500 to-emerald-500 rounded-3xl overflow-hidden mb-8">
                {{-- Decorative --}}
                ...
                <div class="relative px-8 pt-8 pb-6">
                    ...
                    <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-6">
                        <div>
                            ...
                            <h1 class="text-2xl sm:text-3xl font-extrabold text-white leading-tight">{{ $session->nama_sesi }}</h1>
                            @if ($session->deskripsi)
                                <p class="mt-2 text-teal-100 text-sm leading-relaxed max-w-2xl">{{ $session->deskripsi }}</p>
                            @endif
                        </div>
                        <div class="flex-shrink-0">
                            ...
                            @else
                                <form action="{{ route('subdirektorat-inovasi.dosen.inovchalenge.submissions.store', $session) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="inline-flex items-center px-5 py-2.5 bg-white text-teal-700 font-bold text-sm rounded-xl hover:bg-teal-50 transition shadow-sm"
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                        <div class="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-3">
                            ...
                            <p class="text-white text-sm font-bold">{{ $session->periode_awal->format('d M') }} — {{ $session->periode_akhir->format('d M Y') }}</p>
                        </div>
                        <div class="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-3">
                            ...
                            <p class="text-white text-sm font-bold">
                                @if ($session->dana_minimal && $session->dana_maksimal)
                                    Rp {{ number_format($session->dana_minimal / 1000000, 0) }}jt — {{ number_format($session->dana_maksimal / 1000000, 0) }}jt
                        ...
                        <div class="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-3">
                            ...
                            <p class="text-white text-sm font-bold">{{ $session->min_anggota ?? 1 }} – {{ $session->max_anggota ?? 4 }} orang</p>
                        </div>
                        <div class="bg-white/15 backdrop-blur-sm rounded-xl px-4 py-3">
                            <p class="text-teal-200 text-[10px] font-semibold uppercase tracking-wider mb-0.5">Sisa Waktu</p>
                            <p class="text-white text-sm font-bold">
                                @if ($daysLeft > 0)
                                    {{ $daysLeft }} hari lagi
                                @else
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <span class="text-xs text-gray-400 font-medium">{{ $openTahap }}/{{ $totalTahap }} tahap dibuka</span>
                </div>
                ...
                <div class="space-y-4">
                    @foreach ($session->tahap as $tahap)
                        @php
                            ...
                            $stepColors = [
                                1 => ['bg' => 'from-blue-500 to-blue-600', 'light' => 'bg-blue-50 text-blue-700 border-blue-200', 'ring' => 'ring-blue-200'],
                                2 => ['bg' => 'from-purple-500 to-purple-600', 'light' => 'bg-purple-50 text-purple-700 border-purple-200', 'ring' => 'ring-purple-200'],
                                3 => ['bg' => 'from-orange-500 to-orange-600', 'light' => 'bg-orange-50 text-orange-700 border-orange-200', 'ring' => 'ring-orange-200'],
                            ];
                            $sc = $stepColors[$tahap->tahap_ke] ?? $stepColors[1];

                            $statusConfig = match($ts) {
                                'dibuka' => ['label' => 'Dibuka', 'color' => 'bg-green-100 text-green-700', 'icon' => 'fa-unlock', 'dot' => 'bg-green-500'],
                                'ditutup' => ['label' => 'Ditutup', 'color' => 'bg-gray-100 text-gray-500', 'icon' => 'fa-ban', 'dot' => 'bg-gray-400'],
                                default => ['label' => 'Belum Dibuka', 'color' => 'bg-red-50 text-red-500', 'icon' => 'fa-lock', 'dot' => 'bg-red-400'],
                            };

                            $daysLeftTahap = null;
                            if ($tahap->periode_akhir && $isOpen) {
                                $daysLeftTahap = max($now->diffInDays($tahap->periode_akhir, false), 0);
                            }
                        @endphp
                        ...
                        <div class="flex gap-4 {{ $isLocked ? 'opacity-50' : '' }}">
                            ...
                            <div class="flex flex-col items-center flex-shrink-0 w-10">
                                <div class="w-10 h-10 rounded-2xl bg-gradient-to-br {{ $sc['bg'] }} flex items-center justify-center text-white font-bold text-sm shadow-sm {{ $isOpen ? 'ring-4 ' . $sc['ring'] : '' }}">
                                    @if ($isClosed)
                                ...
                                @if (!$loop->last)
                                    <div class="w-0.5 flex-1 mt-2 {{ $isOpen || $isClosed ? 'bg-gray-300' : 'bg-gray-200' }} rounded-full"></div>
                                @endif
                            </div>

                            {{-- Card --}}
                            <div class="flex-1 bg-white rounded-2xl border {{ $isOpen ? 'border-gray-200 shadow-sm' : 'border-gray-100' }} p-5 mb-1">
                                <div class="flex items-start justify-between gap-3 mb-2">
                                    <div>
                                        <h4 class="text-base font-bold text-gray-900">{{ $tahap->judul ?? $tahap->nama_tahap }}</h4>
                                        @if ($tahap->deskripsi)
                                            <p class="text-sm text-gray-500 mt-0.5 leading-relaxed">{{ $tahap->deskripsi }}</p>
                                        @endif
                                    </div>
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-lg text-[10px] font-bold flex-shrink-0 {{ $statusConfig['color'] }}">
                                        <i class="fas {{ $statusConfig['icon'] }} mr-1 text-[8px]"></i> {{ $statusConfig['label'] }}
                                    </span>
                                </div>
                                ...
                                <div class="flex flex-wrap items-center gap-2 mt-3">
                                    @if ($tahap->periode_awal && $tahap->periode_akhir)
                                        <span class="inline-flex items-center gap-1 text-xs text-gray-400">
                                            ...
                                            {{ $tahap->periode_awal->format('d M') }} — {{ $tahap->periode_akhir->format('d M Y') }}
                                        </span>
                                    @endif
                                    <span class="inline-flex items-center gap-1 text-xs text-gray-400">
                                    ...
                                    @if ($daysLeftTahap !== null)
                                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-bold {{ $daysLeftTahap <= 3 ? 'bg-red-50 text-red-600' : 'bg-teal-50 text-teal-700' }}">
