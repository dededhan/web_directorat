...
@section('content')
    ...
    <div x-data="{
        ...
        openModal(title, action, isLink = false) { // Parameter diubah menjadi isLink
            this.modalTitle = title;
            this.formAction = action;
            this.isModalOpen = true;
            this.isLinkOnly = isLink; // Set state berdasarkan parameter
        }
    }">
                                        ...
                                        <tbody>
                                            @forelse ($module->subChapters as $subChapter)
                                                @php
                                                    ...
                                                    $isLinkType = str_contains(strtolower($subChapter->nama_sub_bab), 'link');
                                                @endphp
                                                <tr class="bg-white border-b">
                                                     ...
                                                     <td class="px-6 py-4">
                                                        @if($userFile)
                                                             @if($userFile->type === 'file')
                                                                <span class="inline-flex items-center px-2 py-1 bg-sky-100 text-sky-800 text-xs font-medium rounded-full"><i class='bx bxs-file-pdf mr-1'></i> File Terunggah</span>
                                                             @else
                                                                 <span class="inline-flex items-center px-2 py-1 bg-indigo-100 text-indigo-800 text-xs font-medium rounded-full"><i class='bx bx-link mr-1'></i> Link Tersimpan</span>
                                                             @endif
                                                        @else
                                                            <span class="inline-flex items-center px-2 py-1 bg-gray-100 text-gray-800 text-xs font-medium rounded-full">Belum Ada</span>
                                                        @endif
                                                    </td>
                                                    <td class="px-6 py-4 text-right space-x-2">
                                                        ...
                                                        @if($userFile)
                                                            ...
                                                            @if($isSesiAktif)
                                                                <button @click="openModal('Edit: {{ $subChapter->nama_sub_bab }}', '{{ route('subdirektorat-inovasi.dosen.equity.files.store', ['submission' => $submission->id, 'subChapter' => $subChapter->id]) }}', {{ $isLinkType ? 'true' : 'false' }})" class="font-medium text-white bg-yellow-500 hover:bg-yellow-600 px-3 py-1 rounded-md text-xs">Edit</button>
                                                                <form action="{{ route('subdirektorat-inovasi.dosen.equity.files.destroy', $userFile->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data ini?');">
                                                                    @csrf @method('DELETE')
                                                        ...
                                                        @else
                                                            @if($isSesiAktif)
                                                                @if($isLinkType)
                                                                    <button @click="openModal('Masukkan Link: {{ $subChapter->nama_sub_bab }}', '{{ route('subdirektorat-inovasi.dosen.equity.files.store', ['submission' => $submission->id, 'subChapter' => $subChapter->id]) }}', true)" class="font-medium text-white bg-indigo-600 hover:bg-indigo-700 px-3 py-1 rounded-md text-xs">Masukkan Link</button>
                                                                @else
                                                                    <button @click="openModal('Unggah Dokumen: {{ $subChapter->nama_sub_bab }}', '{{ route('subdirektorat-inovasi.dosen.equity.files.store', ['submission' => $submission->id, 'subChapter' => $subChapter->id]) }}', false)" class="font-medium text-white bg-blue-600 hover:bg-blue-700 px-3 py-1 rounded-md text-xs">Unggah</button>
                                                                @endif
                                                            @else
        ...
        <div x-show="isModalOpen" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100" x-transition:leave="transition ease-in duration-200" x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 px-4" style="display: none;">
            <div @click.away="isModalOpen = false" class="bg-white rounded-lg shadow-xl w-full max-w-lg p-6 sm:p-8" x-show="isModalOpen" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90">
                ...
                <form :action="formAction" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div x-show="!isLinkOnly" x-transition>
                        ...
                        <input type="file" name="file_dokumen" id="file_dokumen" accept=".pdf" class="mt-1 block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100">
                    ...
                    <div x-show="isLinkOnly" x-transition>
                        <label for="url" class="block text-sm font-medium text-gray-700">Masukkan URL</label>
                         <input type="url" name="url" id="url" placeholder="https://docs.google.com/document/d/..." class="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
