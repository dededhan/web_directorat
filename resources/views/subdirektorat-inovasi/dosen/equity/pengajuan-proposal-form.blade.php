...
<style>
    ...
    .tagify:hover { 
        --tags-border-color: #14b8a6; /* border-teal-500 */
    }
    .tagify.tagify--focus {
        --tags-border-color: #14b8a6; /* border-teal-500 */
    }
...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8"
        ...
        <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
            <form method="POST" action="{{ route('subdirektorat-inovasi.dosen.equity.proposal.storePengajuan', $submission->id) }}">
                @csrf
                @method('PUT')
                ...
                <div class="p-6 lg:p-8">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            @error('judul') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="tahun_usulan" name="tahun_usulan" value="{{ old('tahun_usulan', $submission->tahun_usulan ?? date('Y')) }}" 
                            ...
                            @error('tahun_usulan') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        <div>
                            ...
                            <input type="text" id="tempat_pelaksanaan" name="tempat_pelaksanaan" value="{{ old('tempat_pelaksanaan', $submission->tempat_pelaksanaan) }}" 
                            ...
                            @error('tempat_pelaksanaan') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            @error('abstrak') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" id="kata_kunci" name="kata_kunci" x-ref="keywords" value="{{ json_encode(old('kata_kunci', $submission->kata_kunci ?? [])) }}" 
                                   placeholder="Ketik kata kunci lalu tekan Enter">
                            @error('kata_kunci') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        
                        {{-- SDGs --}}
                        <div class="md:col-span-2">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class='bx bx-globe-alt mr-1 text-pink-500'></i> Tujuan Pembangunan Berkelanjutan (SDGs)
                            </label>
                            ...
                            <div class="relative">
                                <div class="w-full bg-white border-2 border-gray-200 rounded-xl p-2 min-h-[50px] flex flex-wrap gap-2 items-center cursor-pointer transition-all duration-200 @error('sdgs') border-red-500 focus-within:border-red-500 focus-within:ring-red-500 @else focus-within:ring-2 focus-within:ring-teal-500 focus-within:border-teal-500 @enderror" 
                                    ...
                                    <span x-show="selectedSdgs.length === 0" class="text-gray-400 text-sm px-2">Pilih satu atau lebih SDGs...</span>
                                </div>
                                <div x-show="sdgDropdownOpen" @click.away="sdgDropdownOpen = false" x-transition 
                                     class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-xl py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm" style="display: none;">
                            ...
                            @error('sdgs') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" id="mitra_nasional" name="mitra_nasional" x-ref="mitra_nasional" value="{{ json_encode(old('mitra_nasional', $submission->mitra_nasional ?? [])) }}" 
                        ...
                        <div>
                            <label for="mitra_internasional" class="block text-sm font-semibold text-gray-700 mb-2">
                                <i class='bx bxs-plane-alt mr-1 text-gray-500'></i> Mitra Internasional (Opsional)
                            </label>
                            <input type="text" id="mitra_internasional" name="mitra_internasional" x-ref="mitra_internasional" value="{{ json_encode(old('mitra_internasional', $submission->mitra_internasional ?? [])) }}" 
                                   placeholder="Ketik nama mitra lalu tekan Enter">
                        </div>
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                ...
                                <input type="text" name="nominal_usulan" id="nominal_usulan" @input="formatNominal($event)" value="{{ old('nominal_usulan', number_format($submission->nominal_usulan ?? 0, 0, ',', '.')) }}" 
                            ...
                            @error('nominal_usulan') <p class="text-red-500 text-xs mt-2 flex items-center"><i class='bx bx-error-circle mr-1'></i>{{ $message }}</p> @enderror
                        </div>
                    </div>
                </div>
