<div x-data="{
        ...
        mobileOpen: false,
        inovChalengeOpen: {{ request()->routeIs('subdirektorat-inovasi.alumni.inovchalenge.*') ? 'true' : 'false' }},
        init() {
    ...
    <div x-show="mobileOpen || window.innerWidth >= 1024"
        ...
        <nav class="flex-1 space-y-3 py-6 px-4 overflow-y-auto">
            ...
            <div>
                ...
                <a href="#"
                   class="flex items-center space-x-4 rounded-lg p-3 transition-colors hover:bg-gray-700"
            ...
            <div class="pt-3">
                ...
                <button @click="inovChalengeOpen = !inovChalengeOpen"
                    ...
                    <div x-show="open || mobileOpen" class="flex items-center">
                        <i class='bx bx-chevron-down text-2xl transition-transform' :class="{'rotate-180': inovChalengeOpen}"></i>
                    </div>
                </button>

                <div x-show="inovChalengeOpen && (open || mobileOpen)" x-collapse class="mt-2 ml-3 space-y-1">
                    <a href="{{ route('subdirektorat-inovasi.alumni.inovchalenge.invitations.index') }}"
                       class="flex items-center space-x-4 rounded-lg p-3 text-sm transition-colors ml-4
                              {{ request()->routeIs('subdirektorat-inovasi.alumni.inovchalenge.invitations.*')
                                  ? 'bg-teal-600 font-semibold text-white shadow-md'
                                  : 'hover:bg-gray-700' }}">
                        <i class='bx bxs-envelope text-2xl flex-shrink-0'></i>
