                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
                    <div class="p-6">
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            {{-- Info --}}
                            <div class="flex-1">
                                <h3 class="text-lg font-semibold text-gray-900">{{ $invitation->submission->session->nama_sesi }}</h3>
                                <p class="text-sm text-gray-500 mt-1">
                                    <i class="fas fa-user mr-1"></i> Diundang oleh:
                                    <span class="font-medium text-gray-700">{{ $invitation->submission->user->name }}</span>
                                </p>
                                ...
                                <div class="mt-2">
                                    <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border {{ $badgeColors[$invitation->approval_status] ?? '' }}">
                            ...
                            @if($isPending)
                                <div class="flex gap-2 flex-shrink-0">
                                    <form method="POST" action="{{ route('subdirektorat-inovasi.alumni.inovchalenge.invitations.approve', $invitation) }}">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit"
                                    ...
                                    <form method="POST" action="{{ route('subdirektorat-inovasi.alumni.inovchalenge.invitations.reject', $invitation) }}"
            ...
            @empty
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
                    <div class="flex flex-col items-center">
                        <div class="w-20 h-20 bg-gradient-to-br from-indigo-100 to-indigo-200 rounded-2xl flex items-center justify-center mb-4">
