...
@section('title', 'Edit Form - ' . ucfirst(str_replace('_', ' ', $form->phase)))

@section('page-title', 'Edit Form Builder')
    ...
    <div class="space-y-6" x-data="formBuilder()">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-600 mt-1">Edit form untuk {{ ucfirst(str_replace('_', ' ', $form->phase)) }}</p>
                <p class="text-xs text-gray-500 mt-1">Last updated: {{ $form->updated_at->format('d M Y H:i') }}</p>
            </div>
        ...
        @if ($session->status == 'active')
            <div class="bg-yellow-50 border-l-4 border-yellow-500 p-4 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div>
                        <h3 class="font-semibold text-yellow-800">Session is Active</h3>
                        <p class="text-sm text-yellow-700 mt-1">
                            Be careful when editing this form as it may affect ongoing submissions.
                            Consider the impact on participants who are already using this form.
                        </p>
                    </div>
                </div>
            </div>
        ...
        <!-- Main Form -->
        <form action="{{ route('admin.inov_challenge.forms.update', ['session' => $session->id, 'form' => $form->id]) }}"
                                ...
                                <template x-if="['select', 'radio', 'checkbox'].includes(fields[selectedFieldIndex].type)">
                                    <div>
                                        ...
                                        <div class="space-y-2" x-data="{ newOption: '' }">
                                            <template
                                                x-if="!fields[selectedFieldIndex].options || Object.keys(fields[selectedFieldIndex].options).length === 0">
            ...
            <div class="flex items-center justify-between bg-white rounded-xl shadow-md p-6 mt-6">
                ...
                <div class="flex items-center space-x-3">
                    <a href="{{ route('admin.inov_challenge.forms.preview', ['session' => $session->id, 'form' => $form->id]) }}"
                        target="_blank"
                        class="px-6 py-2 border border-indigo-600 text-indigo-600 rounded-lg hover:bg-indigo-50 font-medium">
